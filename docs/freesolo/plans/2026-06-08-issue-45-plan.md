...
**Goal:** Add an autonomous approval policy so `freesolo plan generate` optionally auto-transitions `planned → approved` and records a `team.planned` event when `autonomous_mode` is enabled.
...
| Path | Responsibility |
|---|---|
| `src/config/types.ts` (modify) | Add `autonomous_mode: boolean` to `FreesoloConfig` |
...
```ts
// src/config/types.ts — add to FreesoloConfig
...
async function readRepoAutonomousMode(
  ...
  const repoConfigPath = path.join(repoRoot, '.freesolo', 'config.yaml');
  try {
