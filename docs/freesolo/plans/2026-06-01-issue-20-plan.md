...
**Goal:** Build a configurable, independent verification pipeline that runs an ordered list of checks against the repo, persists structured pass/fail results per issue, and exposes it as `freesolo verify`.

**Architecture:** Add a `src/verification/` module containing zod-validated config, a deterministic runner, and a local filesystem store. Wire it into a new `freesolo verify` subcommand that resolves the issue id (override → session state → branch name), reads `freesolo.config.json`, runs each check sequentially via execa with per-check logs, and writes `run.json` under `.git/freesolo/verifications/issue-<N>/<runId>/`. The pipeline is additive: no existing schemas, commands, or host assets change.
...
- Create `tests/integration/verify-command.test.ts` — end-to-end with a temp git repo, real `freesolo.config.json`, real subprocesses.
...
The store uses `git rev-parse --git-path` to resolve `.git/freesolo/...` paths so it stays correct inside git worktrees. Tests create a real git repo with `execa` so `rev-parse` works.
...
async function makeRunDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'freesolo-verify-runner-'));
  tempDirs.push(dir);
  return dir;
}
...
const defaultDeps: ResolveIssueNumberDeps = {
  readSessionFile: async (worktreePath) => {
    try {
      ...
      // (e.g. `.git/freesolo/session.json`). Resolve it against the worktree root so
...
export async function createVerifyPlan(
  ...
  try {
    ...
    if (error instanceof Error && error.message.startsWith('freesolo must be started inside a git repository')) {
      return { mode: 'error', message: error.message, exitCode: 2 };
    }
...
git commit -m "Add freesolo verify command"
```
...
git commit -m "Register freesolo verify subcommand"
```
...
This test creates a real temp git repo, writes a real `freesolo.config.json` with one passing and one failing check using `node -e`, runs the verify command through `createVerifyPlan` with default deps, and asserts on the on-disk artifacts.
...
git commit -m "Add integration test for freesolo verify"
```
...
- **Spec coverage**
  - "Checks are configurable per repo / project" — covered by Tasks 1, 2 (schema + loader, `freesolo.config.json`).
  ...
  - Persistence path `.git/freesolo/verifications/issue-<N>/<runId>/` — covered by Tasks 3, 8.
...
- **Scope** — purely additive. No edits to `session-state.ts`, the workflow kernel, or host integration assets. Wiring the agent into `freesolo verify` automatically is left for the follow-up Verification Gate ticket.
