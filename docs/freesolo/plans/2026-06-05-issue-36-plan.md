...
**Goal:** Ship a local polling watcher (`freesolo watch`) that detects GitHub issues with a configured trigger label, enqueues them in SQLite, drains the queue via the workflow engine, and persists a per-repo cursor across restarts.

**Architecture:** Minimal shared SQLite bootstrap (`~/.freesolo/state.db`) with migration 001 for watcher tables. Config loaded from `~/.freesolo/config.yaml`. Poll module wraps `gh issue list`, runner orchestrates poll → enqueue → drain → cursor advance with rate-limit backoff. CLI exposes `watch run` and `watch once`.
...
| Path | Responsibility |
|---|---|
| `src/config/types.ts` (new) | `WatcherConfig`, `FreesoloConfig`, defaults |
...
- **`FREESOLO_ENGINE` gate applies to `watch run` only** — the spec Engine Integration section says the watch command itself does not require the env var (line 160), but the CLI Surface section gates `run` (line 179). Resolve by gating `run` only: `once` drains without the gate (useful for cron/tests); in-process `createWorkflowEngine().tick()` bypasses the CLI gate anyway — the gate is operational, not enforced inside the engine.
...
export async function loadConfig(configPath = defaultConfigPath()): Promise<FreesoloConfig> {
  let content: string;
  try {
...
export function defaultStateDbPath(): string {
  return process.env.FREESOLO_STATE_DB ?? path.join(os.homedir(), '.freesolo', 'state.db');
}
...
import { MIN_INTERVAL_SECONDS, type FreesoloConfig } from '../config/types.js';
...
| `FREESOLO_ENGINE` gate on `run` only | Task 6 gate + Accepted Design Trade-offs note |
