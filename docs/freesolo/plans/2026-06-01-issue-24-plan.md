...
**Architecture:** Thin orchestrator on top of the existing state machine (#17) and agent adapter (#33). One pure policy function, one orchestrator with synchronous event subscribers, one CLI subcommand gated by `FREESOLO_ENGINE=1`. No daemon, no scheduling, no persistence beyond GitHub labels — restart resumes implicitly.
...
- Create: `src/commands/engine.ts` — `registerEngineCommands`, `EngineCommandDeps`, gated `freesolo engine tick` subcommand
...
These choices were raised during plan review and are kept deliberately; each is documented so an implementer (or a future reviewer) does not relitigate them mid-flight.

- **Tasks 3 and 5 are coverage-locking, not red-green-refactor.** They lock contracts (`wait` semantics in Task 3; subscriber multiplexing, error-swallowing, and unsubscribe in Task 5) that Task 2's implementation already satisfies, and their `Expected: PASS` outcomes reflect that. They stay separate from Task 2 because each contract is independently meaningful and folding both sets of assertions into Task 2 would balloon a single commit beyond what the plan's TDD-per-task rhythm tolerates.
- **Test-file imports are hoisted at the top of `tests/unit/workflow-engine.test.ts`.** `InvalidTransitionError`, `AgentAdapter`, `AgentResponse`, and `AgentStatus` are imported in Task 2 Step 1 even though they are not referenced until Tasks 4 and 6, because ES modules require imports at the top of the file and the project has no lint rule enforcing per-task import locality today. A future ADR enabling `noUnusedLocals` or ESLint `no-unused-vars` will require migrating each import to the task that first uses it.
- **CLI stderr refusal format diverges from the spec's illustrative wording.** The plan's `formatRefusal` produces `engine refused (<code>): <reason>` (e.g. `engine refused (no-state): issue has no state label`) instead of the spec's illustrative `triaged refused: no-state`. The plan's format is preferred because it includes the refusal code explicitly; the spec wording is illustrative ("e.g."), and the CLI tests assert via substring containment so both formats pass.
- **`WriteChannel` is duplicated between `state.ts` and `engine.ts` as parallel construction.** Extraction into a shared `src/commands/io.ts` will land when a third command needs the alias; until then, duplicating a two-member string union keeps the two command modules independently readable.
- **Task 8's spawn happy-path test routes a synthetic `TickResult` through Commander to lock the `formatSuccess` spawn branch.** It is a format-string test via the CLI plumbing, not an end-to-end spawn assertion, because the default CLI deps never wire an agent adapter today (a spawn action would otherwise always refuse with `no-agent-adapter`). The inline comment on that test already says so; the test will continue to lock the format string once a future runner ticket supplies an adapter.
...
export function defaultPolicy(input: PolicyInput): EngineAction {
  ...
  if (input.state === 'closed') {
    // The engine short-circuits before reaching policy on terminal states, but
    // the totality test still calls the policy here; surface a reason that
    // reads correctly if it ever leaks through.
...
All later tasks append additional `describe` blocks to this same file; the imports below already include every symbol that later tasks reference (`InvalidTransitionError` for Task 4, `AgentAdapter`/`AgentResponse`/`AgentStatus` for Task 6). This keeps every `import` at the top of the file as ES modules require.
...
> **Note on unused imports.** `InvalidTransitionError` (used in Task 4) and the `AgentAdapter`/`AgentResponse`/`AgentStatus` triple (used in Task 6) are imported here so every `import` sits at the top of the file. The project's vitest/TypeScript configuration tolerates briefly-unused imports during the red phase of TDD; do not add a placeholder test to "consume" them. If a future lint pass treats unused imports as an error, prefer a single `// eslint-disable-next-line @typescript-eslint/no-unused-vars` on the offending line over a no-op assertion. If a future ADR enables `noUnusedLocals` or adds ESLint with `no-unused-vars`, migrate these imports to the task that first uses them.
...
// Deliberate semantics: `tick` never throws for the typed errors documented in the spec
// (MultipleStateLabelsError, InvalidStateLabelError, InvalidTransitionError — all surfaced
// as TickResult.refused). Unexpected I/O failures from deps (e.g. `agent.start` throwing,
// `agent.send` throwing, or `readState`/`writeState` throwing a non-typed error) propagate
// out of `tick` unchanged. The spec's refusal-code list is closed; introducing
// `agent-failed` would be a spec change, not a fix. A future runner ticket owns retry /
// supervision and may choose to catch these throws there.
//
// State-store typed errors must be caught inside `tick`: the CLI maps refusal codes to
// exit codes and assumes nothing typed escapes. If a future change lets one escape, the
// CLI silently downgrades it to exit 1.
export function createWorkflowEngine(deps: WorkflowEngineDeps): WorkflowEngine {
  ...
  return {
    ...
    async tick({ repo, issueNumber }) {
      ...
      // Stages beyond the state refusals are added in later tasks. Surface every
      // non-wait/non-refusal action as `policy-refused` so the intermediate commit's
      // contract is internally consistent (each later task replaces this branch:
      // Task 4 handles `transition`, Task 6 handles `spawn`, Task 7 finalises `refuse`).
...
Run: `rtk npm test -- --run tests/unit/workflow-engine.test.ts`
Expected: PASS — 4 refusal-case tests.
...
Run: `rtk npm test -- --run tests/unit/workflow-engine.test.ts`
Expected: FAIL — `writeState` is never called and no transition event is emitted because Task 2's engine surfaces every non-wait policy action as a `policy-refused` placeholder instead of actually executing the transition.
...
Append the following block to `tests/unit/workflow-engine.test.ts`. The `AgentAdapter`/`AgentResponse`/`AgentStatus` imports are already at the top of the file (added in Task 2 Step 1); do **not** add another import statement here.
...
describe('createWorkflowEngine tick: spawn action', () => {
  it('refuses with no-agent-adapter when policy asks for spawn but none is configured', async () => {
    ...
    expect(harness.writeState).not.toHaveBeenCalled();
    // Spec contract: exactly one decision event fires even on refusal.
    expect(harness.events.filter((event) => event.kind === 'decision')).toHaveLength(1);
    expect(harness.events.filter((event) => event.kind === 'transition')).toHaveLength(0);
  });
  ...
  it('starts the agent, sends the initial instructions once, then writes the next state', async () => {
    ...
    expect(agent.startCalls).toEqual([
      { workingDirectory: '/tmp/wt', initialInstructions: 'continue freesolo' }
    ]);
    expect(agent.sendCalls).toEqual(['continue freesolo']);
  ...
  it('lets adapter errors during start propagate (engine is one-shot, no retry)', async () => {
    // Per the engine.ts header comment: adapter errors are not one of the six refusal
    // codes in the spec. They escape `tick` so the caller (and a future runner) can
    // decide how to handle them. This test locks that contract.
...
Add the following `it(...)` block **inside the existing `describe('buildCli', …)` in `tests/unit/cli.test.ts`** (immediately after the `'registers the state command group with get and transition subcommands'` test). Leave the existing tests untouched.
...
describe('freesolo engine tick happy paths', () => {
  ...
  it('prints the spawn summary on stdout and exits 0', async () => {
    // The default CLI deps never wire an agent adapter, so a spawn action always
    // refuses with `no-agent-adapter` in production today. This test injects a
    // successful spawn TickResult directly to lock the formatSuccess spawn branch
    // for the future runner ticket that will supply an adapter.
    const { program, io } = buildHarness({
      tick: vi.fn().mockResolvedValue(
        tickResult({
          fromState: 'approved',
...
describe('freesolo engine tick refusal exit codes', () => {
  ...
  it('exits 4 for malformed-state refusals', async () => {
    // The fixture reason mirrors the actual MultipleStateLabelsError message format the
    // engine surfaces in production, so the CLI test exercises a representative payload
    // rather than a synthetic string that happens to contain the asserted substring.
    const { program, io } = buildHarness({
      tick: vi.fn().mockResolvedValue(
        tickResult({
          fromState: null,
...
Create `src/commands/engine.ts`. Note the inline `withCommanderErrorHandling` helper — it mirrors the same-named helper in `src/commands/state.ts`, so unexpected throws from `resolveRepoRef` or `tick` (for example, the "freesolo must run inside a git repository" error) print a clean stderr message and exit `1` rather than leaking a raw stack via Commander's exit override.
...
// Build a fresh engine per CLI invocation. The spec frames the engine as stateless
// between calls, and constructing here (rather than at module-load time) avoids
// sharing the subscriber Set across invocations in the same process.
...
function formatSuccess(result: TickResult): string {
  ...
  // `formatSuccess` is only called when `!result.refused`, and the engine always
  // sets `refused` whenever it emits a `refuse` action. Reaching this branch means
  // an upstream invariant is broken — surface it loudly rather than printing a
...
// Mirrors src/commands/state.ts: turn unexpected throws (e.g. resolveRepoRoot rejecting
// because we are outside a git repo) into a clean stderr line plus exit code 1, instead
// of letting Commander's exit override surface a raw stack. Signature matches state.ts
// exactly so a reader scanning both files side-by-side sees the same shape.
...
Run: `rtk npm test`
Expected: PASS — every suite green, no prior test regresses.
