...
export async function getDecompositionPath(worktreePath: string): Promise<string> {
  const raw = await getFreesoloPath(worktreePath, 'decomposition.json');
...
export async function getDecompositionAppliedPath(worktreePath: string): Promise<string> {
  const raw = await getFreesoloPath(worktreePath, 'decomposition-applied.json');
  return path.isAbsolute(raw) ? raw : path.join(worktreePath, raw);
}
...
// generate: FREESOLO_ENGINE gate exit 3; success prints "decomposition preview written:";
...
// approve: FREESOLO_ENGINE gate exit 3;
...
//   FREESOLO_AUTONOMOUS=1 + FREESOLO_ENGINE=1 succeeds same as human approve
```
...
**generate** (`FREESOLO_ENGINE=1` required):
...
**approve** (`FREESOLO_ENGINE=1` required; no interactive confirmation; `FREESOLO_AUTONOMOUS=1` does not add gates):
