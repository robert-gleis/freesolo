...
Spec for issue [#52](https://github.com/robert-gleis/freesolo/issues/52). Parent: epic #9 (Team Orchestration). See [ADR-0002](../../adr/0002-llm-planner-via-adapter.md).
...
This ticket does **not** wire either feature into the workflow engine or the CLI — those touch `state-machine.ts`, add CLI commands, and write `.git/freesolo/team-plan.json` / `decomposition.json`. They are #34 and #37. The scope here is the planner library that #34, #37, and #45 will import.
...
- **No workflow-engine wiring.** State transitions, the `planned → approved` gate, and `<.git>/freesolo/*.json` writes are #34 / #37.
- **No CLI commands.** `freesolo plan show/edit/approve` and `freesolo decomposition show/edit/approve` are #34 / #37.
...
4. The Team Lifecycle Manager writes the JSON to `.git/freesolo/team-plan.json` and drives the state transition.
...
Public API (importable as `import { ... } from 'freesolo/planner'` once a build mapping exists; today via relative paths inside the repo) consists of:
