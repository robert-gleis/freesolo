...
**Issue:** [#22 — Factory Knowledge Base](https://github.com/robert-gleis/freesolo/issues/22)
...
Add a version-controlled Knowledge Base of repo-specific operational facts under `.freesolo/knowledge/*.md` and inject those entries into every agent's startup instructions at spawn time. Agents pick up edits on the next spawn with no factory restart. Agents running without FreeSolo can read the same files directly.

## Goals

- Store operational knowledge as free-form Markdown, one topic per file, under `.freesolo/knowledge/*.md`.
- Load knowledge from disk at spawn time (no caching) and prepend a structured section to the agent startup prompt.
- Wire injection into both spawn paths today: `freesolo start` (adapter launch) and the workflow engine's `spawn` action (`AgentAdapter.start` / `send`).
...
- A CLI subcommand for listing or editing knowledge (`freesolo knowledge …`). Files are edited directly; a CLI can land in a follow-up.
...
- Only `*.md` files directly under `.freesolo/knowledge/` are loaded. Subdirectories are ignored in v1.
...
| Version-controlled and reviewable | Files live under `.freesolo/knowledge/` in the repo |
| Updatable without restarting the factory | Loader reads from disk on every spawn; no in-process cache |
| Agents without FreeSolo can read files directly | Plain Markdown on disk; ADR-0001 documents the layout |
