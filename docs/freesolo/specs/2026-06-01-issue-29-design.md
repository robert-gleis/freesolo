...
**Issue:** [#29 — Verification Gate](https://github.com/robert-gleis/freesolo/issues/29)
...
- Keep gate evaluation engine-gated (`FREESOLO_ENGINE=1`) so only the workflow engine (or an authorised runner invoking engine commands) can advance past verification.
- Keep PR-creation checks independent of agent self-reporting — `freesolo pr create` re-validates state and verdict before calling `gh pr create`.

## Non-Goals

- Running the verification pipeline itself (that is `freesolo verify` from #20).
- Generating reviewer artifacts from verification output (separate ticket under #12).
- Auto-running verification on every engine tick (the agent or runner still invokes `freesolo verify` explicitly).
- Hooking or patching the bare `gh pr create` binary — enforcement is through `freesolo pr create`.
...
|-----------|------------------------------|
| PR cannot be created if verification fails | `freesolo pr create` refuses when workflow state is not `pr-ready`, when no passing verdict exists, or when the latest run status is not `pass`. Exit code `1` with a message naming the blocking reason. |
| Gate verdict is recorded against the issue | Each evaluation writes `verification:pass` or `verification:fail` on the GitHub issue (mutually exclusive labels, swapped atomically) and persists `gate-verdict.json` under `.git/freesolo/verifications/issue-<N>/`. |
| Gate failures put the issue into a state with a clear next action | Failed evaluations transition `verifying → implementing` and the verdict record includes a `nextAction` string (e.g. "Fix failing checks, run `freesolo verify`, then re-run `freesolo gate evaluate`."). CLI prints the same string on stderr. |
...
src/commands/
  gate.ts              # freesolo gate evaluate (engine-gated)
  pr.ts                # freesolo pr create (gate-enforced wrapper around gh)
```

The workflow engine itself does not grow new action kinds. Gate evaluation is a dedicated CLI command the engine (or runner) calls while the issue is in `verifying`. State transitions use the existing `writeState` path from #17, invoked only when `FREESOLO_ENGINE=1`.

### Data flow

1. Agent finishes implementation review and moves the GitHub issue to `state:verifying` (via engine).
2. Agent runs `freesolo verify` — pipeline writes a run under `.git/freesolo/verifications/issue-<N>/<runId>/`.
3. Engine (or authorised operator) runs `freesolo gate evaluate --issue <N>`:
...
4. Agent runs `freesolo pr create --title ... --body ...`:
...
|-----------|-----------|--------------|
| `latestRun === null` | `no-run` | `Run freesolo verify for this issue, then freesolo gate evaluate.` |
| `latestRun.status === 'pass'` | `pass` | `Create a pull request with freesolo pr create.` |
| `latestRun.status === 'fail'` | `fail` | `Fix failing checks, run freesolo verify, then freesolo gate evaluate.` |
...
Path: `.git/freesolo/verifications/issue-<N>/gate-verdict.json`
...
### `freesolo gate evaluate --issue <number>`

Engine-gated (`FREESOLO_ENGINE=1` required; exit `3` without it — same pattern as `freesolo state transition` and `freesolo engine tick`).
...
### `freesolo pr create [gh pr create options...]`
...
|-----------|-----------|------------------------|
| Not `FREESOLO_ENGINE=1` on `gate evaluate` | `3` | none |
...
3. **Block bare `gh pr create` via git hooks** — rejected as fragile and out of scope; FreeSolo enforces through its own PR command and `pr-ready` state.
