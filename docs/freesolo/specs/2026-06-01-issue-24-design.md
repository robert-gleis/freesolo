...
**Issue:** [#24 — Create Workflow Engine](https://github.com/robert-gleis/freesolo/issues/24)
...
A single new CLI command, gated symmetrically with `freesolo state transition`:

```
freesolo engine tick --issue <number>
```

- Requires `FREESOLO_ENGINE=1` (the same gate the state-transition command uses). Without it, the command prints a clear error and exits with code `3`.
...
- **`tests/unit/engine-command.test.ts`** (new) and **`tests/unit/cli.test.ts`** (modify)
  - `engine tick` requires `FREESOLO_ENGINE=1`; without it, prints an engine-gate error and exits `3`.
