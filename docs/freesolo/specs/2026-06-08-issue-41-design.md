...
**Issue:** [#41 — Team Lifecycle Manager](https://github.com/robert-gleis/freesolo/issues/41)
...
v1 ships a `TeamLifecycleManager` domain module under `src/teams/`, a small CLI surface (`freesolo team start | status | stop`), and `ScriptedAgentAdapter`-based member factories for deterministic tests. Real host adapters plug in through the same factory injection point without changing manager logic.

## Goals

- Create team members from a schema-valid `TeamDefinition` read from `<.git>/freesolo/team-plan.json`.
...
- **Workflow engine policy changes.** v1 does not add a `spawn-team` engine action or modify `defaultPolicy`. Team start/stop is triggered explicitly via `freesolo team` CLI (same pattern as `freesolo plan generate` in #34). A runner ticket wires engine policy later.
...
src/teams/
  ...
  store.ts          # optional team-runtime.json snapshot (read/write via getFreesoloPath)
src/commands/
...
6. Write runtime snapshot to `<.git>/freesolo/team-runtime.json` (best-effort).
...
- `getTeamRuntimePath(worktreePath)` → `getFreesoloPath(worktreePath, 'team-runtime.json')`.
...
```
freesolo team start  --issue <N>   # read team-plan.json, create team, approved → implementing
freesolo team status --issue <N>   # print runtime snapshot (from manager or team-runtime.json)
freesolo team stop   --issue <N>   # tearDown('cancelled')
```
...
6. `writeState(repo, issue, 'approved', 'implementing')` with `FREESOLO_ENGINE=1`.
...
| Engine gate | `FREESOLO_ENGINE` not set on state write | stderr gate message, exit `3` |
...
- **Monitor loop ownership.** v1 CLI starts the team but does not run `monitor()` inline. The spec assumes a future runner process calls `monitor()`. If we need end-to-end manual testing in v1, add `freesolo team run` as a thin `create + monitor` wrapper — left as follow-up unless user wants it in scope.
