...
**Issue:** [#40 — Implement Codex Adapter](https://github.com/robert-gleis/freesolo/issues/40)  
...
**Status:** Approved via freesolo continuation (user review gate)
...
- Replacing `src/adapters/codex.ts` (`buildCodexLaunchPlan`) — that module stays for one-shot host launch via `freesolo start --tool codex`.
