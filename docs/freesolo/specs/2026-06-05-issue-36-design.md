...
**Issue:** [#36 — GitHub Issue Watcher](https://github.com/robert-gleis/freesolo/issues/36)
...
Add a local polling watcher that detects GitHub issues carrying a configured trigger label (default `state:triaged`) and enqueues them for the workflow engine. The watcher runs inside the FreeSolo CLI — no webhooks, no public endpoint. A per-repo cursor in SQLite (`~/.freesolo/state.db`) records the last processed `updatedAt` timestamp so restarts do not re-queue issues. Configuration lives in `~/.freesolo/config.yaml`.
...
- Make polling interval and trigger label configurable via `~/.freesolo/config.yaml`.
- Back off polling when `gh` returns HTTP 403 or 429 (rate limit).
- Ship as `freesolo watch` with `run` (blocking loop) and `once` (single poll, for tests and cron).

## Non-Goals

- Webhook receiver or any network-facing endpoint.
- Multi-repo fan-out in v1 — the watcher operates on the repo resolved from the current checkout's `origin` remote (same as `freesolo engine tick`).
- Full Event Log (#23) or Worktree Metadata (#28) tables — only the shared DB shell and watcher tables land here.
- Spawning agent sessions (`freesolo start`) from the watcher — v1 enqueues by engine tick only. The engine's policy decides the next action.
...
src/
  config/
    load.ts              # read ~/.freesolo/config.yaml (minimal YAML subset)
    types.ts             # WatcherConfig, FreesoloConfig
  state/
  ...
  commands/
    watch.ts             # freesolo watch run | once
```
...
Config path override: `FREESOLO_CONFIG` env var, defaulting to `~/.freesolo/config.yaml`.

## SQLite Schema (migration 001)

Database path: `~/.freesolo/state.db` (override via `FREESOLO_STATE_DB`).
...
**Cursor semantics:** On first run for a repo, `last_seen_updated_at` defaults to the current UTC time (not epoch zero) so the watcher does not flood-queue every historical triaged issue. Operators who want a backfill run `freesolo watch once --since <ISO>` (CLI flag overrides cursor for that run only).
...
1. Set `FREESOLO_ENGINE=1` in the subprocess environment (the watch command itself does not require the env var; only engine transitions do).
...
The watch command does **not** call `freesolo state transition` or write labels directly.

## CLI Surface

```
freesolo watch run [--interval <seconds>] [--trigger-label <label>]
freesolo watch once [--since <iso8601>]
```
...
`run` requires `FREESOLO_ENGINE=1` in the environment (same gate as `engine tick`) because draining the queue invokes the engine.
...
- Export `loadConfig(path?: string): FreesoloConfig`.
...
| Configurable interval and trigger label | `~/.freesolo/config.yaml` + CLI overrides |
...
Ship approach B: poll loop with SQLite queue and shared DB bootstrap. Expose `freesolo watch run` and `freesolo watch once`. Keep the watcher thin — poll, enqueue, drain via engine tick, advance cursor.
