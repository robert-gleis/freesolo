...
**Issue:** [#44 — Merge Readiness Check](https://github.com/robert-gleis/freesolo/issues/44)
...
Introduce a **Merge Readiness Gate** — the final factory checkpoint before a pull request may be merged. The gate evaluates verification, review, PR provenance, and optional policy checks, persists an authoritative verdict, surfaces the checklist on the PR itself, and blocks `freesolo merge` until every required check passes. Re-running verification or the workflow after a fix invalidates a stale ready verdict and allows the gate to be re-evaluated to `ready` once checks pass again.

v1 ships a pure evaluator, verdict persistence (GitHub labels + local JSON + PR comment), and `freesolo merge evaluate|merge|show` CLI. No workflow-engine policy wiring in this ticket — downstream automation calls the evaluator directly, matching #35 and #43.
...
- **Branch protection rule configuration.** FreeSolo enforces through `freesolo merge`; bare `gh pr merge` bypass is documented, same pattern as #29 for `gh pr create`.
- **Workflow engine policy changes.** No `src/workflow/` imports from merge readiness in v1.
- **Auto-merge on green.** v1 evaluates and optionally merges when explicitly invoked with `freesolo merge`; no daemon polling.
- **PR creation.** Covered by #43; merge readiness assumes a PR already exists.
- **Running the verification pipeline.** Callers run `freesolo verify` and `freesolo gate evaluate` (#29) before merge evaluate.
...
|-----------|------------------------------|
| PR is blocked from merging until all gates pass | `freesolo merge` refuses `gh pr merge` unless `evaluateMergeReadiness` returns `outcome: 'ready'`. Exit `1` with per-check failure reasons. |
...
`evaluateMergeReadiness(input)` runs a fixed checklist of gates. `freesolo merge evaluate` writes verdict, swaps labels, syncs PR comment. `freesolo merge` merges only when ready.
...
### B. GitHub Check Run (`freesolo/merge-readiness`)

Create a required status check via Checks API.

**Rejected for v1:** Requires more GitHub API surface, token scopes, and repo configuration; FreeSolo's CLI-first pattern favours comments + labels first.
...
Policy extension (v1 minimal): read optional `.freesolo/merge-policy.json`:
...
- Stale verification: if verdict `runId` ≠ latest run `runId`, `verification-verdict` fails with detail "Stale verdict — re-run `freesolo gate evaluate`."
- `nextAction` when blocked lists the first failing gate's remediation (e.g. "Run `freesolo verify` then `freesolo gate evaluate` then `freesolo merge evaluate`.")
...
Path: `git rev-parse --git-path freesolo/merge-readiness.json`
...
`freesolo merge [--issue <N>] [--merge-method squash|merge|rebase]`:

1. Load latest `merge-readiness.json`; if `outcome !== 'ready'`, re-run evaluation inline.
2. If still blocked → exit `1`, print `nextAction`.
3. Engine-gated (`FREESOLO_ENGINE=1`): `writeState(..., 'pr-ready', 'merged')` **after** successful `gh pr merge` (not before — merge can fail).
...
```
freesolo merge evaluate [--issue <N>] [--print-only]
freesolo merge [--issue <N>] [--merge-method <m>]
freesolo merge show [--issue <N>]
```

`--issue` resolution: same order as `freesolo verify` (flag → session → branch).
...
- #29: `src/verification/gate.ts`, `verdict-store.ts`, `freesolo gate evaluate`
- #43: `src/integration/pr-*.ts`, `pull-request.json`, `freesolo pr create|show`
...
Option A: pure evaluator + verdict persistence + PR comment sync + `freesolo merge` CLI in `src/integration/`, following the #29 verdict-label pattern and #43 provenance pattern. Satisfies all three acceptance criteria with testable, injectable boundaries and explicit re-open semantics via stale-run detection.
