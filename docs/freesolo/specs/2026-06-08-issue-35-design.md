...
**Issue:** [#35 — Candidate Branch Creation](https://github.com/robert-gleis/freesolo/issues/35)
...
- Expose `freesolo candidate create` for manual and scripted invocation.
...
src/integration/
  ...
  store.ts              # readCandidateBranchRecord / writeCandidateBranchRecord via getFreesoloPath
  index.ts              # barrel
...
Path: `git rev-parse --git-path freesolo/candidate-branch.json`

Uses the same `getFreesoloPath` pattern as `session.json` and `team-plan.json`. JSON validated with Zod on read. One record per issue worktree (latest write wins).
...
```
freesolo candidate create --issue <N> --team <teamId> --branches <b1,b2,...> [--base main] [--force]
freesolo candidate show --issue <N>
```

`create` resolves `issueSlug` from the issue packet in the current worktree's `current-issue.md` or from `git rev-parse` freesolo metadata when available; if missing, derives slug from branch name `issue/<N>-<slug>` on the current branch.
