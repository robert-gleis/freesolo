...
**Issue:** [#45 — Autonomous Team Creation](https://github.com/robert-gleis/freesolo/issues/45)
...
Add a **policy layer** over the Team Planner that optionally skips the human `planned → approved` step. When `autonomous_mode` is enabled, `freesolo plan generate` auto-approves the planner output immediately after writing `team-plan.json` and transitioning to `planned`. The decision is recorded in the Event Log as a `team.planned` event referencing the team definition path. When `autonomous_mode` is false (default), behaviour is unchanged — human approval via `freesolo plan approve` remains required.
...
src/config/
  types.ts                  # extend FreesoloConfig with autonomous_mode
...
```yaml
# ~/.freesolo/config.yaml (global default)
autonomous_mode: false

# <repo>/.freesolo/config.yaml (optional per-repo override)
autonomous_mode: true
```

Resolution order (repo wins):

1. Parse `autonomous_mode` from `<repoRoot>/.freesolo/config.yaml` if the file exists.
2. Else parse from `~/.freesolo/config.yaml` (or `FREESOLO_CONFIG` path).
...
Auto-approval inherits the generate command's `FREESOLO_ENGINE=1` gate. No separate gate.
...
- **Config drift.** Repo override may surprise operators who only set global config. Document in spec; future `freesolo config show` can surface effective values.
