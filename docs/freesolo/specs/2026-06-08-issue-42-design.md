...
- Role is visible in stderr logs (`[freesolo] spawn agent=… role=…`) and in `agent.created` event payloads.
...
- **CLI subcommands.** No `freesolo team spawn` in v1. Expansion and prompt building are library APIs tested directly.
...
export interface RoleContextProfile {
  roleName: string;
  /** Reserved: glob patterns under .freesolo/knowledge/ to include. Empty = all. */
...
```
[freesolo] spawn agent=agent-42-backend-engineer-1 role="Backend Engineer" host=cursor instance=1/1 cwd=/path/to/worktree
```
...
#### `freesolo start` (`src/commands/start.ts`)
