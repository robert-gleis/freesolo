...
**Issue:** [#25 — Implement Tmux Runner](https://github.com/robert-gleis/freesolo/issues/25)  
**Parent:** [#11 — Epic: Runner Abstraction](https://github.com/robert-gleis/freesolo/issues/11)  
**Builds on:** [#18 — Create Runner Interface](https://github.com/robert-gleis/freesolo/issues/18) (merged)  
**Status:** Draft
...
- **Wiring `freesolo start` through `TmuxRunner`.** Issue #18 explicitly leaves `start.ts`’s one-shot `execa` handoff out of scope; that path has no supervision or log capture. A future ticket may choose tmux vs local process at the CLI layer.
...
- Derived from `runner.id`: `freesolo-<sanitized-id>`.
