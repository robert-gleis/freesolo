...
- **Output written to `<.git>/freesolo/team-plan.json`** — Met. `getTeamPlanPath` delegates to `getFreesoloPath(worktreePath, 'team-plan.json')` (`store.ts:15-17`), which resolves via `git rev-parse --git-path freesolo`. Store writes pretty-printed JSON with trailing newline and creates parent dirs (`store.ts:20-24`). Round-trip and path-resolution tests use real `git init` temp repos.
- **Plans inspectable/overridable via `freesolo plan` CLI** — Met. Four subcommands registered in `src/cli.ts:52` and `src/commands/plan.ts:204-351`: `generate`, `show`, `edit`, `approve`. `--issue` is optional via `resolveIssueNumber`; `show` pretty-prints JSON; `edit` opens `$EDITOR` (default `vi`), validates on save, and leaves the on-disk file untouched when validation fails.
...
| Engine gate | `FREESOLO_ENGINE !== '1'` → exit `3` | generate, approve |
...
## Path resolution
- `getTeamPlanPath` resolves through `git rev-parse --git-path freesolo/team-plan.json` and normalises relative git paths to absolute under the worktree (`store.ts:15-17`). This is a sensible hardening over the plan's bare `getFreesoloPath` call — `git rev-parse --git-path` can return a relative path in some worktree layouts.
- `defaultFetchIssue` tries `gh issue view --json number,title,body` first, then falls back to parsing `current-issue.md` from the freesolo dir (`plan.ts:67-96`). The extra `worktreePath` parameter (vs the plan sketch) is required for both paths and is correctly threaded from `resolveIssueContext`.
...
- Engine gate message format is consistent with `state.ts` ("engine-only… Set FREESOLO_ENGINE=1… agent processes must not bypass the workflow engine").
