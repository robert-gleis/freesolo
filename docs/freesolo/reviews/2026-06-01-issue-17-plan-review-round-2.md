...
- Exit codes: `0` success, `1` invalid transition or generic error, `2` no state label on `state get`, `3` missing `FREESOLO_ENGINE`, `4` `MultipleStateLabelsError`. All map to the spec at line 115 and are exercised by tests.
...
- `state-command.test.ts` covers all required exit-code transitions: 1 (invalid transition, unknown --to, uninitialised issue), 2 (null state on get), 3 (missing FREESOLO_ENGINE on transition), 4 (multi-state-labels on both get and transition).
