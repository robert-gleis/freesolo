...
- Persistence path is per-worktree (via `git rev-parse --git-path`), which the spec endorses as matching the `session.json` convention. Worth keeping in mind that running `freesolo verify` from the main checkout vs. a worktree produces separate, non-overlapping run directories — by design, not a finding.
