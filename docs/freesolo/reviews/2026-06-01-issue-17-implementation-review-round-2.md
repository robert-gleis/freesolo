...
- **AC3 — Agents cannot bypass states:** met as a soft gate. `state transition` requires `FREESOLO_ENGINE=1`; without it the command writes a clear stderr message and exits 3 before `readState`/`writeState` is touched (`state-command.test.ts:109-127`).
...
- `FREESOLO_ENGINE` gate runs before `--to` validation, `resolveRepoRef`, and `readState` (`state-command.test.ts:109-127`).
...
- README at `/Users/A15AB98/projects/private/freesolo.issue-17-introduce-workflow-state-machine/README.md:56-68` documents the new commands, exit-2 semantics, env-var gate, and canonical state list.
