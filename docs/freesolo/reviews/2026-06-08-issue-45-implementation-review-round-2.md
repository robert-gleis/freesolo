...
| Task | Status | Notes |
|------|--------|-------|
| Task 1: Extend global config with `autonomous_mode` | **Complete** | `FreesoloConfig`, `DEFAULT_CONFIG`, `parseAutonomousModeFromContent`, `loadConfig` |
...
3. **Document `autonomous_mode` in operator docs.** Spec risk note flags config drift when only global is set. No README/CLI help change in scope; consider follow-up `freesolo config show`.
