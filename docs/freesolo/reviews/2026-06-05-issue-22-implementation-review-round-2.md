...
| Version-controlled and reviewable | Met | Starter files at `.freesolo/knowledge/{conventions,build,test,deploy}.md`; smoke test guards against deletion or gitignore regression. |
...
- Launch test correctly targets the resolved worktree path (`resolveBranchWorktreePath` → `/wt/issue-12-ship-freesolo-start`), matching production `repoRoot = worktreePath` semantics.
