...
- **Integrates with FreeSolo workflow** — Met. Integration test wires `createWorkflowEngine` with mock invoker on spawn path; asserts one invoke, adapter `running`, and full `['decision', 'transition']` event sequence.
...
## Spec alignment (`src/agents/claude-code.ts` vs `docs/freesolo/specs/2026-06-02-issue-39-design.md`)
