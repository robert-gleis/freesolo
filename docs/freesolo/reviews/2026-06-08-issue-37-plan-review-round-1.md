...
- Tasks 1–2 follow strict TDD with complete test files, `git init` in `makeWorktree` helpers (matching `team-plan-store.test.ts` / `plan-command.test.ts`), and path resolution via `getFreesoloPath` with absolute-path join — aligned with `src/planner/store.ts`.
