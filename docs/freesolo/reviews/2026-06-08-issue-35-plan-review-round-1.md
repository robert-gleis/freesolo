...
8. **minor — Store path helper should follow `getTeamPlanPath` absolute-path convention (plan: Task 2; precedent: `src/planner/store.ts`).** `getFreesoloPath` returns a path that may be relative to the worktree; `getTeamPlanPath` resolves via `path.isAbsolute(rawPath) ? rawPath : path.join(worktreePath, rawPath)`. Task 2 mentions `getFreesoloPath` but not this join. Without it, store read/write may fail in real git worktrees where `rev-parse --git-path` returns a relative `.git/freesolo/...` path.
...
- `getFreesoloPath` for `candidate-branch.json` matches `team-plan.json` / `session.json` storage pattern.
