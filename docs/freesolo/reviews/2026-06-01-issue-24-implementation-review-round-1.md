...
- **AC: single source of truth.** `engine.tick` is the only production caller of `writeState` outside of `src/commands/state.ts`, and that surface is itself gated by `FREESOLO_ENGINE=1` (see `src/commands/state.ts:130`). The new engine CLI command (`src/commands/engine.ts:130`) uses the same gate, so both write paths agree on the engine-only contract.
