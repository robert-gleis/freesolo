...
## Acceptance criteria
- **Metadata survives restart** — Met. `getWorktreeStore()` opens `~/.freesolo/state.db` (or `FREESOLO_STATE_DIR`), runs migrations, and `metadata survives restart` round-trips upsert → reopen → read (`tests/unit/state-worktrees.test.ts`).
- **Metadata is source of truth for ownership** — Met. `WorktreeStore` exposes upsert/get/list/delete/touch; `freesolo worktrees list` prints persisted rows; `freesolo start` upserts after worktree resolution and before session writes (`src/commands/start.ts:349-356`).
- **Drift detection (both directions)** — Met. `detectWorktreeDrift` + `loadDriftCandidates` cover `onDiskOnly` and `metadataOnly`; `freesolo worktrees drift` wires git enumeration, path-existence preflight, candidate filtering, and exit code `1` on drift.
...
- `freesolo worktrees list|drift` follow existing command patterns: injectable `WorktreesCommandDeps`, `withCommanderErrorHandling`, `--json` output, drift exit `0`/`1`/`2`, and preflight `pathExists` batching into a sync map before calling pure drift helpers.
- `freesolo start` integration: upsert is ordered before `writeSessionState`; `StateDbError` from upsert aborts the plan before session/packet writes; `startAction` maps `StateDbError` to a clear stderr message and exit `1` without launching the host (`src/commands/start.ts:490-494`).
- `FREESOLO_STATE_DIR` override enables test isolation from `~/.freesolo` across path, DB, and round-trip tests.
