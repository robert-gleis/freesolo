...
| `FREESOLO_ENGINE` gate | `watch run` gated (exit 3); `watch once` ungated per plan trade-off |
...
| `FREESOLO_ENGINE` gate on `run` only | `commands/watch.ts:144` | `watch-command.test.ts` |
