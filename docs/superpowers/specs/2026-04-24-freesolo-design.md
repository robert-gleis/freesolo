...
`freesolo` is a publishable, repo-agnostic CLI that starts a focused development session for one GitHub issue assigned to the current user in the current repository.
...
The session launched by `freesolo` follows this stage model.
...
- `docs/freesolo/specs/YYYY-MM-DD-issue-<number>-design.md`
...
- `docs/freesolo/plans/YYYY-MM-DD-issue-<number>-plan.md`
...
When the user reopens an existing issue worktree, `freesolo` should be able to continue rather than restart blindly.
...
- `freesolo status`
- `freesolo doctor`
- `freesolo tui` or `freesolo dashboard`
...
Build `freesolo` as a CLI-first, host-neutral workflow launcher with a strong shared kernel and thin host adapters. Keep the first version intentionally narrow:
