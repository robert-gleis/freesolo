...
**Goal:** Ship `src/memory/adrs.ts` to scan and load numbered ADRs from `docs/adr/`, inject them into the issue packet at `issueflow start`, export `nextAdrNumber`, and document ADR non-goals in `docs/adr/README.md`.
...
### Task 5: Wire listAdrs into issueflow start
...
git commit -m "$(cat <<'EOF'
Load ADRs at issueflow start and include them in the issue packet.
