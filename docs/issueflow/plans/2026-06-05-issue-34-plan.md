...
**Goal:** Add a team planner that analyses a GitHub issue via `AgentAdapter`, validates structured JSON output, persists it to `<.git>/issueflow/team-plan.json`, and exposes `issueflow plan generate|show|edit|approve` CLI commands with engine-gated state transitions.
...
export async function getTeamPlanPath(worktreePath: string): Promise<string> {
  return getIssueflowPath(worktreePath, 'team-plan.json');
}
...
function buildHarness(overrides: Partial<PlanCommandDeps> = {}): {
  ...
  const deps: PlanCommandDeps = {
    ...
    writeTeamPlan: vi.fn().mockResolvedValue('/repo/.git/issueflow/team-plan.json'),
    getTeamPlanPath: vi.fn().mockResolvedValue('/repo/.git/issueflow/team-plan.json'),
...
describe('issueflow plan edit', () => {
  ...
  it('restores original file when editor output fails validation', async () => {
    const originalPath = '/repo/.git/issueflow/team-plan.json';
    const { program, io, deps } = buildHarness({
...
- `requireEngineGate(subcommand)` — shared helper; exit `3` with message matching state.ts style when `ISSUEFLOW_ENGINE !== '1'`. Only `generate` and `approve` call it; `show` and `edit` do not.
...
git commit -m "Add issueflow plan CLI commands"
```
