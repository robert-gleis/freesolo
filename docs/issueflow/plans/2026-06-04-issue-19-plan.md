...
git commit -m "Add WorktreeManager domain types and error class for issue #19"
```
...
git commit -m "Add WorktreePlacement interface and InMemoryWorktreePlacement reference"
```
...
git commit -m "Add WorktreeManager interface contract"
```
...
export class InMemoryWorktreeManager implements WorktreeManager {
  ...
  async acquire(input: AcquireInput): Promise<WorktreeRecord> {
    ...
    if (existingId !== undefined) {
      ...
      if (!existing) {
        // Indexes diverged — treat as no existing record.
...
Expected: PASS — seven `acquire` tests green. (Tests for `release`, `touch`, `findOrphans`, `reap` will be added in later tasks.)

- [ ] **Step 5: Commit**

```bash
git add src/worktrees/in-memory.ts tests/unit/in-memory-worktree-manager.test.ts
git commit -m "Implement InMemoryWorktreeManager.acquire with idempotency and validation"
```
...
```bash
git add src/worktrees/in-memory.ts tests/unit/in-memory-worktree-manager.test.ts
git commit -m "Implement InMemoryWorktreeManager.release and .touch"
```
...
```ts
  async findOrphans(now?: Date): Promise<WorktreeOrphanReport> {
    ...
    const onDiskByPath = new Map(onDisk.map((loc) => [loc.path, loc]));
    ...
    const orphans: WorktreeOrphan[] = [
      ...
      ...untrackedLocations.map((location): WorktreeOrphan => ({ kind: 'untracked-location', location: onDiskByPath.get(location.path) ?? location }))
    ];
...
```bash
git add src/worktrees/in-memory.ts tests/unit/in-memory-worktree-manager.test.ts
git commit -m "Implement InMemoryWorktreeManager.findOrphans and .reap"
```
...
git commit -m "Expose Worktree Manager public surface via src/worktrees barrel"
```
...
// that ends in `/worktrees` or `/worktrees/<anything>`.
...
git commit -m "Add regression test guarding workflow engine isolation from worktrees module"
```
...
Expected: every existing test still passes; the eight new tests files (worktree-manager-types, in-memory-worktree-placement, in-memory-worktree-manager, worktree-engine-isolation) report a combined ~30+ tests passing.
