...
Build the shared, machine-bound SQLite state store at `~/.issueflow/state.db` that hosts the local telemetry tables consumed by the Event Log (#23), Worktree Metadata (#28), and the Issue Watcher cursor (#36). This ticket delivers infrastructure only — connection management, WAL-mode initialisation, a migration runner, and backup / safe-delete helpers — under a new `src/state-store/` module. The concrete tables (`events`, `worktrees`, watcher cursors) ship in the follow-up tickets.

ADR-0001 is the rationale: telemetry lives locally; knowledge lives in the repo.

## Goals

- Provide a single TypeScript module that any IssueFlow code can call to obtain a ready-to-use SQLite connection rooted at `~/.issueflow/state.db`.
- Enable concurrent IssueFlow processes against the same database via WAL mode and a sensible `busy_timeout`.
...
- No CLI surface (`issueflow state-store …`) in this ticket. Consumers call the module directly.
...
src/state-store/
  ...
  paths.ts            # resolveDefaultPath(), resolveTrashDir() — homedir + ISSUEFLOW_HOME override
...
- Lets future tooling (CLI `issueflow state-store backup` etc., #51 follow-ups) import the same surface without restructuring.
...
Default target path when `targetPath` is omitted: `<source>.backup-<UTC-ISO>.db` — e.g. `~/.issueflow/state.db.backup-2026-06-04T17-23-18-499Z.db`. The timestamp uses filename-safe characters (colons and dots in the ISO timestamp replaced with hyphens).
...
2. Create `<ISSUEFLOW_HOME>/trash/<UTC-ISO>/` directory.
...
That is sufficient for IssueFlow's expected load (low-frequency event writes from a handful of agent processes). The integration test (see Testing) drives this scenario explicitly to lock in the behaviour.
...
| File | Coverage |
|---|---|
| `state-store-paths.test.ts` | `resolveDefaultPath` returns `<homedir>/.issueflow/state.db` and honours `ISSUEFLOW_HOME`. |
...
|---|---|
| `~/.issueflow/state.db` is created on first use, with WAL enabled | `openConnection` ensures the parent directory exists and applies `journal_mode = WAL` on every open. `state-store-connection.test.ts` asserts both behaviours. |
...
| Tests cover concurrent-writer scenarios (two IssueFlow processes appending events) | `state-store-concurrency.test.ts` spawns N child processes, each writing rows in parallel against a shared DB. |
...
- **Trash directory grows forever.** `safeDelete` never cleans `<ISSUEFLOW_HOME>/trash/`. Acceptable: the operator chose to delete the DB, so they can `rm -rf` when ready. If automatic cleanup ever matters, a `--keep-last N` flag on a future CLI command would solve it.
...
- **No CLI surface in this ticket.** A follow-up ticket can add `issueflow state-store backup|trash|info` if operator ergonomics demand it. Out of scope here.
