...
- Make writes idempotent under concurrent IssueFlow processes (safe upsert, WAL mode).
- Wire `issueflow start` so metadata is upserted automatically when a worktree is created or reused.
...
- Replacing `.git/issueflow/session.json` — session state remains per-worktree intra-session state.
...
|---|---|---|
| #19 Worktree Manager | Closed | `src/core/worktree.ts` and `issueflow start` already manage worktree lifecycle via Worktrunk. |
...
Add `better-sqlite3`, open `~/.issueflow/state.db` with WAL, run numbered SQL migrations, expose a `WorktreeStore`. Hook into `issueflow start`.
...
### B. JSON files under `~/.issueflow/worktrees/`
...
```
src/state/
  paths.ts          # resolveStateDbPath() → ~/.issueflow/state.db
...
src/commands/worktrees.ts   # issueflow worktrees list | drift
```
...
- `upsert` is a single statement — safe to call from concurrent `issueflow start` processes.
...
`issueflow worktrees drift` resolves the repo root (same helper as other commands), calls `listWorktreeEntries(repoRoot)`, loads all DB rows whose `path` is in the git entry path set **or** whose `path` is missing from disk, and returns the diff. Rows in DB for paths outside the current repo's worktree list are included in `metadataOnly` when the path no longer exists on disk (`fs.access`).
...
```
issueflow worktrees list [--json]
issueflow worktrees drift [--json]
```
...
No `sync` command in v1 — `issueflow start` is the write path. A future ticket can add bulk reconciliation.

## Integration: `issueflow start`
...
- Integration test: temp `ISSUEFLOW_STATE_DIR` env override pointing at a temp directory (see paths module) so tests never touch `~/.issueflow`.
...
`resolveStateDbPath()` checks `process.env.ISSUEFLOW_STATE_DIR` first (absolute directory containing `state.db`). Production default remains `~/.issueflow/state.db`. Documented in code comment only — not exposed as CLI flag.

## Acceptance Criteria Mapping

| Criterion | How |
|---|---|
| Metadata survives restart | SQLite file at `~/.issueflow/state.db`; tests round-trip upsert → reopen → read. |
| Metadata is source of truth for ownership | Documented; `list` and store API exposed; `start` writes on every attach/create. |
| Drift detection (both directions) | `detectWorktreeDrift` + `issueflow worktrees drift`. |
