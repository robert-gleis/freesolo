...
**Issue:** [#21 — Architecture Decision Records (ADRs)](https://github.com/robert-gleis/issueflow/issues/21)
**Parent:** #14 — Epic: Operational Memory
**Status:** Draft, awaiting user review

## Summary

Wire up ADRs as first-class operational memory: a small `src/memory/` module scans `docs/adr/` for numbered decision files, and `issueflow start` injects the loaded set into the issue packet at spawn time. Conventions (`docs/adr/`, `ADR-FORMAT.md`, ADR-0001/0002) already exist from commit `55f014f`; this ticket delivers the loader, spawn-time injection, a `nextAdrNumber` helper, and explicit non-goal documentation for review findings.
...
- Agents running without IssueFlow can still read ADRs as plain Markdown files in the repo (unchanged; injection is additive).
- Review findings and agent audit events are explicitly documented as **non-goals** for ADRs.

## Non-Goals

- **Review findings store.** PR comments and `docs/issueflow/reviews/*` artifacts stay where they are. ADRs are not a home for ephemeral review output.
- **Agent audit / telemetry.** Planner choices, spawn events, and lifecycle telemetry belong in the Event Log (#23), not ADRs.
- **ADR authoring CLI.** No `issueflow adr new` command in v1. Authors create files manually following `ADR-FORMAT.md` and use `nextAdrNumber()` (exported for tests and future CLI) to pick the next id.
...
- **Knowledge Base injection (#22).** Sibling ticket; `src/memory/` is structured so `.issueflow/knowledge/` can land beside ADRs later without reshaping this module.
...
| C. Separate `.git/issueflow/adr-snapshot.md` | Keeps `current-issue.md` smaller | Extra file to discover; duplicates packet pattern |
...
Injection is written to `.git/issueflow/current-issue.md` via existing `writeIssuePacket`. Hot-update: editing an ADR and re-running `issueflow start` (or manually rewriting the packet) picks up changes — no daemon restart.
...
- Explicit **not stored here** list: review findings (`docs/issueflow/reviews/`, PR threads), agent audit events (Event Log / #23).
- Note that plain-file readability works with or without IssueFlow.
...
| Readable without IssueFlow | Unchanged; README states this |
