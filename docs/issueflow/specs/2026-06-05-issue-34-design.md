...
**Issue:** [#34 — Team Planner](https://github.com/robert-gleis/issueflow/issues/34)
...
Add a team planner that analyses a GitHub issue and produces a structured team definition — which roles are needed, how many of each, which agent host runs each role, and what each role is responsible for. The planner runs through the standard `AgentAdapter` interface, validates its output against a JSON schema, persists the result to `<.git>/issueflow/team-plan.json`, and exposes CLI commands to inspect, edit, and approve the plan. Approval drives the existing `planned → approved` workflow state transition.
...
- Persist team plans at `<.git>/issueflow/team-plan.json` (resolved with `git rev-parse --git-path issueflow/team-plan.json`).
- Expose `issueflow plan show`, `issueflow plan edit`, and `issueflow plan approve` CLI subcommands.
...
- **Engine policy changes.** The default `defaultPolicy` stays `wait` for `triaged`. Generation is triggered explicitly via `issueflow plan generate`, not by `engine tick`.
...
src/planner/
  ...
  store.ts       # readTeamPlan / writeTeamPlan via getIssueflowPath
...
- `getTeamPlanPath(worktreePath)` — resolves via `getIssueflowPath(worktreePath, 'team-plan.json')`.
...
State writes use the existing `writeState` from `src/workflow/state-store.ts`, gated the same way as `issueflow state transition`:
...
Both commands check `ISSUEFLOW_ENGINE=1`. Without it, print a clear error (matching the engine-gate message style) and exit `3`. This keeps agents from bypassing the engine in production while allowing tests to set the env var.
...
```
issueflow plan generate --issue <N>   # run planner, write team-plan.json, triaged → planned
issueflow plan show     --issue <N>   # print team-plan.json to stdout
issueflow plan edit     --issue <N>   # open team-plan.json in $EDITOR, re-validate on save
issueflow plan approve  --issue <N>   # validate schema, planned → approved
```
...
5. `writeState(repo, issue, 'triaged', 'planned')` (with `ISSUEFLOW_ENGINE=1`).
...
3. `writeState(repo, issue, 'planned', 'approved')` (with `ISSUEFLOW_ENGINE=1`).
...
| Engine gate | `ISSUEFLOW_ENGINE` not set | stderr gate message, exit `3` |
...
| Output written to `<.git>/issueflow/team-plan.json` | `store.ts` uses `getIssueflowPath`. |
| Plans inspectable/overridable via `issueflow plan` CLI | `show`, `edit`, `approve` subcommands as specified. |
...
Trigger planning via `engine tick` spawn when state is `triaged`. Rejected because the issue specifies explicit `issueflow plan` CLI commands and ADR-0002 treats planner output as a human-reviewable proposal. Explicit `plan generate` keeps the human-in-the-loop flow clear.
