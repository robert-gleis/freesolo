...
**Issue:** [#50 — Implement Cursor Adapter](https://github.com/robert-gleis/issueflow/issues/50)  
...
Add `CursorAgentAdapter`, a concrete `AgentAdapter` implementation that drives Cursor Agent sessions headlessly via the installed `cursor-agent` CLI. The adapter gives the workflow engine (#24) the same lifecycle surface (`start` / `stop` / `send` / `status`) as future Pi, Claude Code, and Codex adapters, while preserving today's `issueflow start --tool cursor` launch-plan behaviour.
...
- Wiring `CursorAgentAdapter` into `issueflow start` or the workflow engine in this ticket — callers pass the adapter when configuring the engine.
- `@cursor/sdk` dependency — the CLI is already required for `issueflow start --tool cursor` and matches the passive launch plan.
...
`src/adapters/cursor.ts` is **unchanged** — it continues to build the declarative `LaunchPlan` used by `issueflow start`.
...
| Integrates with IssueFlow workflow | Engine `spawn` path works when adapter is injected; no engine changes required |
| Same status surface as other adapters | Uses shared `AgentStatus` / `AgentState` types; same fields as reference adapter |
| No regressions in Cursor-driven flows | `buildCursorLaunchPlan` untouched; `issueflow start --tool cursor` launch plan identical |
