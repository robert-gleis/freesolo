...
**Issue:** [#43 — Automated Pull Request Creation](https://github.com/robert-gleis/issueflow/issues/43)
...
src/integration/
  ...
  pr-store.ts           # read/write pull-request.json via getIssueflowPath
...
Path: `git rev-parse --git-path issueflow/pull-request.json`
...
`--issue` resolution order (same as `issueflow verify`):
1. `--issue` flag
...
Option A: artifact-driven PR body builder + injectable `gh` creator in `src/integration/`, provenance at `pull-request.json`, thin `issueflow pr` CLI. Satisfies all three acceptance criteria while enforcing verification gate at creation time without workflow engine coupling.
