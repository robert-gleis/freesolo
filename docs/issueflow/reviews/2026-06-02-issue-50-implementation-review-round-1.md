...
4. **nit — No engine + `CursorAgentAdapter` integration test.** Spec non-goals explicitly defer wiring into `issueflow start` or the workflow engine; engine spawn compatibility is architectural (existing `workflow-engine.test.ts` spawn tests use a fake adapter). Unit tests with injected `run` satisfy the spec.
