...
- **Integrates with IssueFlow workflow** — Met. `tests/unit/claude-code-workflow-integration.test.ts` wires `createWorkflowEngine` with a mock invoker on the `spawn` path; asserts one invoke with spawn prompt, adapter remains `running`, and a `transition` event is emitted.
