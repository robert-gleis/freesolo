...
- Config resolution order is correct: repo `.issueflow/config.yaml` first, then global via `loadConfig(defaultConfigPath())` (respects `ISSUEFLOW_CONFIG`).
