...
**Spec:** `issueflow reports show` exits `2` when issue cannot be resolved.
...
| 4 Store + session artifacts | Complete (impl) / partial (tests) | `gitIssueflowPath` mirrors verification store; ENOENT + unwritable tests absent |
