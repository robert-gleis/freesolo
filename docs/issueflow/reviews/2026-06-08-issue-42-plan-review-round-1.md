...
3. **major — Task 6 Step 5 understates spawn-test breakage; multiple existing tests need expectation updates (plan: Task 6 Step 5; `tests/unit/workflow-engine.test.ts`).** Step 5 only says to update the `starts the agent` test. After role framing, at least two more spawn tests will fail as written:
   - `enriches spawn instructions with knowledge entries before start and send` asserts `enriched` contains `continue issueflow` and `## Factory Knowledge Base` but not the role section; ordering should be role → knowledge per spec.
...
| `issueflow start` log spawn line + optional `agent.created` | Task 7 | Partial (log only — finding #2) |
