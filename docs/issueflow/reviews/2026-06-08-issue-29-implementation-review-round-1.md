...
2. **Important — `fail` outcome does not print `nextAction` to stderr**
   ...
   ```
   to stdout. The spec requires: "CLI prints the same string on stderr" (referring to `nextAction`, e.g. "Fix failing checks, run `issueflow verify`, then `issueflow gate evaluate`."). Agents and operators watching stderr for next-action guidance will not see it.
