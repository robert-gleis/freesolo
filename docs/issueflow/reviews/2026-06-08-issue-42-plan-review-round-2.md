...
| 2 | major | Task 7 omits optional `agent.created` for `issueflow start` | **Resolved.** Task 7 Step 3 calls `deps.appendEvent?.({ eventType: 'agent.created', … })`; Step 1 adds `appendEvent` to `StartPlanDeps`; Step 4 adds integration test asserting payload when dep is provided. |
...
| 8 | minor | Log-format tests lack full field-order assertion | **Resolved.** Task 5 `team-spawn.test.ts` asserts the complete spec line via `toBe('[issueflow] spawn agent=… role="…" host=… instance=… cwd=…')`. |
...
| `issueflow start` default Implementer role, `host: input.tool` | Task 7 | Covered |
| `issueflow start` log spawn line + optional `agent.created` | Task 7 | Covered |
