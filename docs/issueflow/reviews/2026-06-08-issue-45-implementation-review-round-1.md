...
| Task | Status | Notes |
|------|--------|-------|
| Task 1: Extend global config with `autonomous_mode` | **Complete** | `IssueflowConfig`, `DEFAULT_CONFIG`, `parseAutonomousModeFromContent`, `loadConfig`; 3 new config-load tests |
...
6. **Document `autonomous_mode` in operator docs.** Spec risk note flags config drift when only global is set. No README/CLI help change in scope; consider follow-up `issueflow config show`.
