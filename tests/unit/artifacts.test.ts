...
describe('findIssueArtifacts', () => {
  it('discovers existing spec, plan, and review artifacts for the selected issue', async () => {
    ...
    await fs.mkdir(path.join(repoRoot, 'docs/freesolo/plans'), { recursive: true });
    await fs.mkdir(path.join(repoRoot, 'docs/freesolo/reviews'), { recursive: true });

    const specPath = path.join(repoRoot, 'docs/freesolo/specs/2026-04-20-issue-12-design.md');
    const planPath = path.join(repoRoot, 'docs/freesolo/plans/2026-04-21-issue-12-plan.md');
    ...
    await fs.writeFile(path.join(repoRoot, 'docs/freesolo/specs/2026-04-20-issue-99-design.md'), '# other');
  ...
  it('discovers report artifacts from the freesolo reports directory', async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'freesolo-artifacts-'));
    tempDirs.push(repoRoot);

    await execa('git', ['init'], { cwd: repoRoot });

    const reportsDir = path.join(repoRoot, '.git', 'freesolo', 'reports', 'issue-12');
